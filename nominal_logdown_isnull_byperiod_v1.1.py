...
try:
    ...
    last_year = first.replace(year=prev_year)

    # Create and configure logger
    str_today = datetime.datetime.now().strftime("%Y-%m-%d")
    log_file = "nominal_logdown_isnull_" + str_today + ".log"
    logging.basicConfig(filename=log_file,
                        format='%(asctime)s %(message)s',
                        filemode='a')

    # Running app
    print("-- Fetch logdown data from database Nominal 1 with stop_at is null v1.1 --")                    
    ...
    connection2 = mysql.connector.connect(host='localhost',
                                         ...
                                         user='root',
                                         password='')
    # logger.debug("Opened MySQL connection2 host='locahost' db='recon2'")

    start_period = last_year
    end_period = first
    ...
    try:
        sql_truncate = "TRUNCATE tb_alarm_jarkom_isnull"
        cursor2 = connection2.cursor()
        cursor2.execute(sql_truncate)
        connection2.commit()
        
        sql_insert = """
            INSERT INTO tb_alarm_jarkom_isnull
        ...
        connection2.commit()
        logger.info("Record inserted successfully into tb_alarm_jarkom_isnull table. %d rows affected" %(cursor2._rowcount))
    except mysql.connector.errors.Error as e:
        logger.error("Query failed", e)
    finally:
        cursor2.close()
