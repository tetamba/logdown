...
log_file = "nominal_calculate_logdown_v1.1.1.2_" + str_today + ".log"
logging.basicConfig(filename=log_file,
                    format='%(asctime)s %(message)s',
                    filemode='a')

# Running app
print("-- Calculate logdown downtime from database recon v1.1.1.2 --")                    
...
logger.info("Running script v1.1.1.2")
...
for i in range(1, 13):
    ...
    for index, row in df2.iterrows():
        id_jarkom = row['id_jarkom']
        period = datetime.datetime.strptime(row['period'], "%Y-%m-%d").date()
        offline = 0
        diff = daily_second - offline
        if diff <= 0:    
            av = 0 
